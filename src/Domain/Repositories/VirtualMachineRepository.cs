...
public class VirtualMachineRepository : EntityRepository<VirtualMachine>
{
    ...
    private void SeedData()
    {
        VirtualMachine vm1 = new VirtualMachine.VirtualMachineBuilder()
            ...
            .SetAvailabilities(
                new HashSet<Availability> { Availability.Saturday, Availability.Monday }
            )
            .SetBackupFrequenty(BackupFrequenty.Daily)
            .SetCredentials(
                new HashSet<Credentials>() { new("Admin", "password"), new("User", "password") }
            )
            .SetDuration(new Duration(DateTime.Now, DateTime.Now.AddMonths(1)))
            .SetFqdn("com.vic.vm1")
            .SetHost(new Server("server1", new Resources(4, 4, 4)))
            .SetMode(Mode.IAAS)
            ...
            .SetRequester(
                new InternalCustomer(
                    "Toegepaste Informatica",
                    "DIT",
                    new ContactPerson("Kerem", "Yilmaz", "kerem.yilmaz@valid.com", "0483447325"),
                    new ContactPerson("Kerem", "Yilmaz", "kerem.yilmaz@valid.com", "0483447325")
                )
            )
            ...
            .SetUser(
                new InternalCustomer(
                    "Toegepaste Informatica",
                    "DIT",
                    new ContactPerson("Kerem", "Yilmaz", "kerem.yilmaz@valid.com", "0483788945"),
                    new ContactPerson("Kerem", "Yilmaz", "kerem.yilmaz@valid.com", "0483447325")
                )
            )
            .Build();
        ...
        VirtualMachine vm2 = new VirtualMachine.VirtualMachineBuilder()
            ...
            .SetFqdn("com.vic.vm2")
            .SetHost(new Server("server2", new Resources(4, 4, 4)))
            .SetMode(Mode.PAAS)
            .SetPorts(new HashSet<Port>() { new(1, "port1"), new(2, "port2") })
            ...
            .SetRequester(
                new InternalCustomer(
                    ...
                    new ContactPerson(
                        "Angela",
                        "Degryse",
                        "angela.degryse@valid.com",
                        "0483567812"
                    ),
                    new ContactPerson("Angela", "Degryse", "angela.degryse@valid.com", "0483567812")
                )
            )
            .SetResource(new Resources(2, 2, 2))
            .SetStatus(Status.Requested)
